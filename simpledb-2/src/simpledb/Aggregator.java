...
public interface Aggregator {
	...
	class MinAggregateFunction implements AggregateFunction {
		 ...
		 */
		public void merge(Field value) {
			if (((IntField) value).getValue() < ((IntField) min).getValue())
			   this.min=value;		
	...
	class MaxAggregateFunction implements AggregateFunction {
		...
		public void merge(Field value) {
			// some code goes here
			if (((IntField) value).getValue() > ((IntField) max).getValue())
				   this.max=value;
			throw new UnsupportedOperationException("Implement this");
		}
	...
	class IntAverageAggregateFunction extends IntSumAggregateFunction {
		 ...
		 */
		public void merge(Field value) {
			int iVal = ((IntField) value).getValue();
			ArrayList<Integer> list = new ArrayList<Integer> ();
			list.add(avg);
			list.add(iVal);
			avg = (list.get(1) + list.get(2)) / 2;
			// some code goes here
			//throw new UnsupportedOperationException("Implement this");
		}
