...
public class StringAggregator implements Aggregator {
	...
	class StringAggregatorImplWithoutGrouping extends StringAggregatorImpl {
		...
		public DbIterator iterator() {
			...
			//aggFtn.get((aggFtn).keySet());
			int result = ((ArrayList<Tuple>) aggFtn).size();
	...
	class StringAggregatorImplWithGrouping extends StringAggregatorImpl {
		...
		public StringAggregatorImplWithGrouping(int gbfield, Type gbfieldtype, int afield, Op what) {
			...
			td = new TupleDesc(type);
			field2aggFtn = (Map<Field, AggregateFunction>) this.aggFtnFactory.createAggregateFunction();
			//throw new UnsupportedOperationException("Implement this");
		}
		...
		public void merge(Tuple tup) {
			// some code goes here
			//throw new UnsupportedOperationException("Implement this");
			AggregateFunction f = field2aggFtn.get(gbfield);
			f.merge(tup.getField(afield));
		}
		...
		public DbIterator iterator() {
			...
			ArrayList<Tuple> tuples = new ArrayList<Tuple>();
    		td = new TupleDesc(type);
    		 Collection<Field> j = field2aggFtn.keySet();
    			for(Field i:j) {
    				Set<Entry<Field, AggregateFunction>> entry = field2aggFtn.entrySet();
    				int value = (int) ((IntField) entry).getValue();
    				IntField f = new IntField(value);
    				Tuple t = new Tuple(td);
    				t.setField(0,i);
    				t.setField(1,f);
    				tuples.add(t);	
