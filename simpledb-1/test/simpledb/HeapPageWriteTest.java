...
public class HeapPageWriteTest extends SimpleDbTestBase {
	...
	public void deleteTuple() throws Exception {
		...
		for (int i = 0; i < tuples.size(); i++) {
			page.deleteTuple(tuples.remove(i));
		}
		System.out.println("Tuples Size:"+tuples.size()+"TupleCount:"+tupleCount(page));
		assertEquals(tuples.size(), tupleCount(page));
		it = page.iterator();
		while (it.hasNext()) {
			assertTrue(TestUtil.compareTuples(it.next(), tuples.remove(0)));
		}
